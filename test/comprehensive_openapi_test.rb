...
class ComprehensiveOpenAPITest < Minitest::Test
  ...
  def test_spec_loads_without_errors
    errors = @document.errors.to_a
    assert errors.empty?, "OpenAPI spec has #{errors.size} errors:\n#{errors.join("\n")}"
  end

  def test_openapi_version_is_311
    assert_equal '3.1.1', @document.openapi
  end
  ...
  def test_production_server_configured
    assert @document.servers.any?
    prod_server = @document.servers.first
  ...
  def test_all_documented_paths_exist
    ...
    paths_to_test.each do |path, description|
      assert @document.paths[path], "Missing path: #{path} (#{description})"
    end
  end
    ...
    @document.paths.each do |path, path_item|
      %i[get post put patch delete].each do |method|
        ...
        operation.responses.each do |code, response|
          ...
          response.content.each do |media_type, media|
  ...
  def test_required_schemas_exist
    ...
    schemas = @document.components.schemas.keys
  ...
  def check_schema_for_31_features(schema, path = '', &block)
    ...
    yield :nullable if schema.any_of&.any? { |s| s.type == 'null' }
